...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result?.ok) {
        router.push("/background-replace");
      } else {
        setError("登录失败，请重试");
      }
    } catch {
  ...
  const handleGitHub = () => signIn("github", { callbackUrl: "/background-replace" });
    ...
    <div className="flex items-center justify-center min-h-[80vh] px-6">
      <div className="w-full max-w-sm">
        <div className="text-center mb-8">
          ...
          <p className="text-xs text-foreground/25">登录你的账号</p>
        </div>

        <Button onClick={handleGitHub} variant="outline" size="lg"
          className="w-full h-11 border-border hover:bg-white/[0.03] mb-4 text-xs font-medium rounded-2xl">
        ...
        <div className="flex items-center gap-3 mb-4">
          ...
          <span className="text-[10px] text-foreground/15">或</span>
          <div className="flex-1 h-px bg-border" />
        </div>
        ...
        <form onSubmit={handleSubmit} className="glass p-6 space-y-4">
          ...
          <Button type="submit" className="w-full h-10 bg-gradient-to-r from-purple-500 to-pink-500 hover:from-purple-600 hover:to-pink-600 border-0 text-sm font-medium rounded-xl" disabled={loading}>
            {loading ? "登录中..." : "登录"}
          </Button>
        </form>
