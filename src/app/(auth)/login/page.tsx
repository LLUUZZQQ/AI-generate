...
export default function LoginPage() {
  ...
  const handleEmailLogin = async (e: React.FormEvent) => {
    ...
    if (result?.ok) { router.push("/login?sent=1"); }
    else if (result?.error) { setError("发送失败，请重试"); }
    setLoading(false);
  };
      ...
      <div className="w-full max-w-sm">
        ...
        {emailSent ? (
          <div className="glass rounded-xl p-8 text-center space-y-4">
            ...
            <h3 className="font-semibold">邮件已发送</h3>
            <p className="text-sm text-white/40">请检查邮箱 <strong className="text-white/70">{email}</strong>，点击邮件中的链接完成登录</p>
            <p className="text-xs text-white/20">没收到？检查垃圾邮件箱</p>
          </div>
        ) : (
          <>
            {/* GitHub login */}
            ...
            <form onSubmit={handleEmailLogin} className="glass rounded-xl p-6 space-y-4">
              <div>
                <label className="text-[11px] font-medium text-white/40 mb-1.5 block">邮箱 Magic Link</label>
